<h1 align="center">Y Naga Vamsi</h1>

<p align="center">
  <strong>Software Engineer</strong><br/>
  Full-Stack · Systems · Applied Cryptography
</p>

<p align="center">
  Building reliable software systems — from production-grade applications
  to execution-safe primitives — with a focus on correctness and long-term design.
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/y-naga-vamsi">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:yadavallinagavamsi@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-EA4335?logo=gmail&logoColor=white"/>
  </a>
</p>

---

## About Me

- Software engineer focused on **systems, correctness, and real products**
- Strong interest in **execution safety, verification, and trust models**
- Comfortable owning **end-to-end architecture**
- Prefer building **things that should not break**

---

## Featured Projects (Public)

### admit-cli · Go
🔗 https://github.com/vamsiy78/admit-cli

Safety-first launcher primitive that blocks process execution unless configuration invariants are satisfied.

- Process **never starts** if config is invalid
- Designed for deterministic, safety-critical startup environments
- Strong signal for **systems & infrastructure thinking**

---

### r0 · TypeScript
🔗 https://github.com/vamsiy78/r0

Cryptographically verifiable proof that a specific human approved a legal action at a specific time.

- Time-bound, tamper-resistant consent model
- Bridges cryptography with real-world legal semantics
- Focus on trust, verification, and auditability

---

### Real-Time Vehicle Number Plate Detection · Python
🔗 https://github.com/vamsiy78/Real-time-vehicle-plate-detection

- Real-time number plate recognition using **OpenCV + OCR**
- Optimized preprocessing for improved accuracy
- Practical applied computer-vision system

---

### Secure Chat Using RSA Encryption
🔗 https://github.com/vamsiy78/RSA_Chat_Implementation

- End-to-end encrypted chat using Python sockets
- RSA-based key exchange and secure messaging
- Focus on confidentiality and integrity

---

### FTP Implementation (Cisco Packet Tracer)
🔗 https://github.com/vamsiy78/FTP_CiscoPacketTracer_Implementation

- FTP simulation demonstrating networking and file transfer fundamentals

---

### VoiceText
🔗 https://github.com/vamsiy78/Voicetext

- Voice-to-text and text-to-speech automation utility

---

## Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?logo=go&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1)

### Frontend
![React](https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?logo=nextdotjs&logoColor=white)
![Tailwind](https://img.shields.io/badge/Tailwind_CSS-06B6D4?logo=tailwindcss&logoColor=white)

### Backend & Systems
![Node.js](https://img.shields.io/badge/Node.js-339933?logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express.js-000000)
![REST](https://img.shields.io/badge/REST_APIs-02569B)

### Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?logo=mysql&logoColor=white)

### Security & Verification
![RSA](https://img.shields.io/badge/RSA-Cryptography-8E44AD)
![Verification](https://img.shields.io/badge/Verification_Models-333333)

### Tools
![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?logo=github)
![Linux](https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black)

---

<p align="center">
  <em>Building systems that should not break.</em>
</p>
